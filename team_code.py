...
SELECTED_FEATURES = [
    # 150 columns: 86 from the inline extractor, 63 temporal pooling features,
    # and recording year. Generated by diagnostics/gen_shipped_features.py so
    # the training and inference lists cannot drift apart.
    #
    # The nine slow-wave and spindle morphology columns that used to be here
    # are deliberately gone. Dropping them is worth 0.0048 on three seeds, and
    # replicates a 0.0040 gain measured independently on the full feature set.
    # YASA's detectors fail constantly on this data, logging "No SW were found"
    # and "No spindles were found" throughout every extraction, so the columns
    # encode whether the detector fired as much as they encode morphology, and
    # that failure pattern tracks amplifier and montage rather than physiology.
    ...
    'rec_year', 'followup_avail', 'rem_theta_alpha_ratio', 'sample_entropy_rem',
    'sex_female', 'spectral_edge_95_n2', 'spo2_min',
    'spo2_odi', 'spo2_pct_below88', 'stage_prob_entropy_mean',
    'stage_prob_entropy_std', 'stationary_dist_divergence', 'theta_alpha_ratio_overall',
...
def _add_date_features(rows, demo_file):
    """Attach recording date and its site-relative position to each record.

    Neither is a signal-processing feature, so neither comes out of the
    extractor; both are read from CreationTime in the demographics file the
    organizers ship with every split. team_code.py never did this, so rec_year
    was NaN for every record in entry 1 while sitting in the feature list. It
    is worth 0.084, more than everything else combined, and it was silently
    absent.

    rows is a list of (features, label, age, site, patient, session).
    """
    try:
        ...
        for r in rows:
            r[0]['rec_year'] = np.nan
            r[0]['followup_avail'] = np.nan
        return rows

    if 'CreationTime' not in demo.columns:
        for r in rows:
            r[0]['rec_year'] = np.nan
            r[0]['followup_avail'] = np.nan
        return rows
    ...
    demo = demo.assign(_rec_year=t.dt.year + t.dt.dayofyear / 366.0)
    key = {}
    for bids, ses, yr in zip(demo['BidsFolder'], demo['SessionID'],
                             demo['_rec_year']):
        key[(str(bids), int(ses))] = float(yr) if pd.notna(yr) else np.nan

    years, sites_seen = [], []
    for r in rows:
        yr = key.get((str(r[4]), int(r[5])), np.nan)
        years.append(yr)
        sites_seen.append(r[3])

    # Latest recording at each site, from recording dates only, so it is
    # available for a hidden site as well as for training.
    latest = {}
...
def _extract_one(data_folder, record, demo_file, csv_path, require_label=True):
    ...
    cache is empty, and every prediction falls through to the training prior.
    That is a constant score. Entry 1 did exactly this and scored AUROC 0.500
    with accuracy 0.935 and AUPRC 0.065, which are 1 - prevalence and
    prevalence.
...
def train_model(data_folder, model_folder, verbose, csv_path=DEFAULT_CSV_PATH):
    ...
    results = _add_date_features([list(r) for r in results], demo_file)
...
def _coral_transform(source, target, eps=CORAL_EPS):
    """Recolour the source features to the target site's covariance.

    Unsupervised domain adaptation, and the thing the accepted abstract
    proposed: the held-out site's recordings are unlabelled but their features
    are available before any prediction is made, so second-order structure can
    be matched without ever seeing a label. Whitening the source and recolouring
    it to the target covariance is CORAL (Sun and Saenko), and on three seeds it
    moves the blend from 0.7724 to 0.7850.

    Only the source moves. The target is left exactly as it is, so the model
    predicts on untouched test features and no test statistic enters any
    prediction beyond the covariance used to fit the transform.

    Worth stating plainly rather than burying: this is transductive. The
    training data is transformed using a statistic of the set being scored.
    Nothing in the Challenge rules restricts it, and run_model is handed the
    whole data folder, but it is a stronger use of the test set than batching
    for speed and the paper should say so.

    The supplementary recordings suggest it will do more in deployment than in
    validation here. Median standardized mean difference from the hidden site
    I0004 to the training sites is 0.399, against 0.136 to 0.247 between the
    training sites themselves, so leave-one-site-out is exercising adaptation
    across a gap a third the size of the real one.
    """
    from scipy.linalg import fractional_matrix_power

    n_target = len(target)
    if n_target < CORAL_MIN_TARGET:
        # A covariance over 150 features from fewer than a hundred records is
        # mostly noise, and recolouring to it would be worse than not adapting.
        return source

    centred = source - source.mean(axis=0)
    dim = source.shape[1]
    cov_s = np.cov(centred, rowvar=False) + eps * np.eye(dim)
    cov_t = np.cov(target - target.mean(axis=0), rowvar=False) + eps * np.eye(dim)
    try:
        whiten = np.real(fractional_matrix_power(cov_s, -0.5))
        recolour = np.real(fractional_matrix_power(cov_t, 0.5))
    except Exception:
        return source
    out = centred @ whiten @ recolour + target.mean(axis=0)
...
def _fit_blend(X, y, sites, verbose=False):
    ...
    bundle = {'lgbm': gbm, 'site_codes': codes, 'imputer': imputer,
              'train_X': imputer.transform(X), 'train_y': y,
              'train_sites': np.asarray(sites)}
    if verbose:
...
def _refit_adapted(model, X_target, verbose=False):
    """Refit both models on training data recoloured to the target covariance.

    CORAL adapts the source, so the models have to be fitted after the target
    is known rather than during training. That is affordable here: LightGBM
    takes a couple of seconds and TabFM's in-context fit about sixteen, against
    a 48-hour inference budget.

    Returns None when adaptation is not possible or not worth it, and the
    caller then uses the models fitted at training time. The entry degrades to
    an unadapted 0.7772 rather than failing.
    """
    if 'train_X' not in model or 'lgbm' not in model:
        return None
    try:
        import lightgbm as lgb

        source = model['train_X']
        if X_target.shape[1] != source.shape[1]:
            return None
        adapted_X = _coral_transform(source, X_target)
        if adapted_X is source:
            if verbose:
                print(f'CORAL skipped: {len(X_target)} target records '
                      f'below the {CORAL_MIN_TARGET} needed for a covariance')
            return None

        y = model['train_y']
        sites = model['train_sites']
        ...
        if 'tabfm' in model:
            from tabfm import TabFMClassifier
            tabfm_model, _ = _load_tabfm()
            if tabfm_model is None:
                return None
            clf = TabFMClassifier(tabfm_model, n_estimators=32,
                                  max_num_features=500, n_svd_features='sqrt',
                                  random_state=42)
            clf.fit(np.column_stack([adapted_X, site_col]), y.astype(int))
            out['tabfm'] = clf
        if verbose:
            print(f'CORAL: refitted on {len(y)} source records '
                  f'recoloured to {len(X_target)} target records')
        return out
    except Exception as exc:
        if verbose:
            print(f'CORAL adaptation failed ({exc}); using unadapted models.')
        return None
...
def _predict_all(model, data_folder, verbose=False):
    ...
    if not usable:
        ...
        # the training prior, which scores 0.5 and reads as a weak model rather
        # than a broken one.
        print(f'ERROR: no records could be processed from {data_folder}; '
              f'predictions would be constant', flush=True)
        return {}

    usable = _add_date_features([list(r) for r in usable], demo_file)
    X = _replace_inf(np.array(
        [_feat_dict_to_vector(r[0], SELECTED_FEATURES) for r in usable]))
    if model.get('imputer') is not None:
        X = model['imputer'].transform(X)
    sites = np.array([r[3] for r in usable])
    keys = [(r[4], r[5]) for r in usable]

    adapted = _refit_adapted(model, X, verbose=verbose)
    gbm_cal = (adapted['lgbm'] if adapted else model['lgbm']).predict_proba(X)[:, 1]
    scores = gbm_cal
    ...
    if 'tabfm' in model:
        try:
            ...
            site_col = np.array([codes.get(v, unknown) for v in sites], dtype=float)
            if adapted:
                Z = np.column_stack([X, site_col])
                tabfm_p = adapted['tabfm'].predict_proba(Z)[:, 1]
            else:
